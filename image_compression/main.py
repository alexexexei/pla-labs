...
if __name__=="__main__":
    ...
    compr_img = None
    if (is_rgb): compr_img = img_utils.compress_rgb_img(img, compr_perc)
    else: compr_img = img_utils.compress_grayscale_img(img, compr_perc)
    
    save_path = img_path.replace(folder_path, compr_folder_path)
    img_utils.save_img(compr_img, save_path)
